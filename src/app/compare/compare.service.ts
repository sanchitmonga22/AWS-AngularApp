...
export class CompareService {
  ...
  onStoreData(data: CompareData) {
    ...
    this.authService.getAuthenticatedUser().getSession((err, session) => {
      ...
      this.http
        .post(
          "https://h0kbdchqy8.execute-api.us-east-1.amazonaws.com/dev/comparison",
          data,
          {
            headers: new Headers({
  ...
  onDeleteData() {
    ...
    this.authService.getAuthenticatedUser().getSession((err, session) => {
      this.http
        .delete(
          "https://h0kbdchqy8.execute-api.us-east-1.amazonaws.com/dev/comparison/?accessToken=XXX",
          {
            headers: new Headers({
              Authorization: session.getIdToken().getJwtToken(),
            }),
